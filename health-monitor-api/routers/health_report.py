...
from llm import open_ai, gemini, groq, biomistral
...
async def get_table_data(query: str, user_id: str, llm_choice: str) -> Dict[str, Any]:
    ...
    try:
        if llm_choice == "openai":
            ...
            logger.debug(f"Using Groq to generate SQL for: {query}")
            sql_query = groq.chat_completion_groq(messages, temperature=0)
        elif llm_choice == "biomistral":
            logger.debug(f"Using BioMistral to generate SQL for: {query}")
            sql_query = biomistral.chat_completion_biomistral(messages, temperature=0)
        else:
    ...
    try:
        ...
        try:
            if request.llm_choice == "openai":
                ...
                logger.debug("Using Groq for final report generation")
                report_content = groq.chat_completion_groq(
                    [{"role": "user", "content": report_prompt}],
                    temperature=0.7,
                )
            elif request.llm_choice == "biomistral":
                logger.debug("Using BioMistral for final report generation")
                report_content = biomistral.chat_completion_biomistral(
                    [{"role": "user", "content": report_prompt}],
                    temperature=0.7,
                )
            else:
