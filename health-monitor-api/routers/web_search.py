...
# Initialize Tavily client only when API key is set (allows app to start without it)
...
async def medical_search(request: MedicalSearchRequest):
    if tavily_client is None:
        raise HTTPException(
            status_code=503,
            detail="Tavily API key not configured. Set TAVILY_API_KEY in .env to use medical search.",
        )
    try:
