...
def format_response_with_llm(sql_query: str, query_results: str, llm_choice: str) -> str:
    ...
    if llm_choice == "openai":
        ...
        formatted_response = groq.format_response_groq(prompt)
    elif llm_choice == "biomistral":
        formatted_response = biomistral.format_response_biomistral(prompt)
    elif llm_choice == "openrouter":
        formatted_response = openrouter.format_response_openrouter(prompt)
    else:
...
def format_rag_response(query: str, context: str, llm_choice: Literal["openai", "gemini", "local", "groq", "biomistral", "openrouter"]) -> str:
    prompt = f"""
    ...
    if llm_choice == "openai":
        ...
        return groq.format_rag_response_groq(prompt)
    elif llm_choice == "biomistral":
        return biomistral.format_rag_response_biomistral(prompt)
    elif llm_choice == "openrouter":
        return openrouter.format_rag_response_openrouter(prompt)
    else:
        raise ValueError("Invalid LLM choice")
