...
async def generate_indian_details(llm_choice: str = "biomistral") -> Dict[str, Any]:
    """Generate realistic Indian medical facility and patient details using OpenAI or Groq"""
    try:
        ...
        if llm_choice == "groq":
            from llm.groq import chat_completion_groq
            messages = [
                {"role": "system", "content": "You are an expert in Indian healthcare administration. Return ONLY valid JSON without any explanation or additional text."},
                {"role": "user", "content": prompt},
            ]
            response_content = chat_completion_groq(messages, temperature=0.7)
        elif llm_choice == "biomistral":
            from llm.biomistral import chat_completion_biomistral
            messages = [
                {"role": "system", "content": "You are an expert in Indian healthcare administration. Return ONLY valid JSON without any explanation or additional text."},
                {"role": "user", "content": prompt},
            ]
            response_content = chat_completion_biomistral(messages, temperature=0.7)
        else:
...
async def generate_medical_content(doc_type: str, llm_choice: str = "biomistral") -> Dict[str, Any]:
    """Generate sample medical content based on document type using OpenAI or Groq"""
    try:
        indian_details = await generate_indian_details(llm_choice)
        common_data = {
        ...
        system_content = """You are an experienced Indian medical professional. 
        ...
        if llm_choice == "groq":
            from llm.groq import chat_completion_groq
            response_content = chat_completion_groq(
                [{"role": "system", "content": system_content}, {"role": "user", "content": prompt}],
                temperature=0.7,
            )
        elif llm_choice == "biomistral":
            from llm.biomistral import chat_completion_biomistral
            response_content = chat_completion_biomistral(
                [{"role": "system", "content": system_content}, {"role": "user", "content": prompt}],
                temperature=0.7,
            )
        else:
