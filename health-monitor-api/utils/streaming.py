...
from llm import groq as groq_llm, biomistral as biomistral_llm, openrouter as openrouter_llm
...
async def stream_formatted_response(sql_query: str, query_results: str, tabular_data: list, llm_choice: str):
    ...
    if llm_choice == "groq":
        # Groq: get full response then yield as stream
        async def generate_formatted_response():
            try:
                text = groq_llm.format_response_groq(prompt)
                for char in text:
                    yield char
    ...
    if llm_choice == "biomistral":
        async def generate_formatted_response():
            try:
                text = biomistral_llm.format_response_biomistral(prompt)
                for char in text:
                    yield char
                yield f"[SQL_QUERY]{sql_query}[/SQL_QUERY]"
                yield f"[TABULAR_DATA]{json.dumps(tabular_data)}[/TABULAR_DATA]"
    ...
    if llm_choice == "openrouter":
        async def generate_formatted_response():
            try:
                text = openrouter_llm.format_response_openrouter(prompt)
                for char in text:
                    yield char
                yield f"[SQL_QUERY]{sql_query}[/SQL_QUERY]"
                yield f"[TABULAR_DATA]{json.dumps(tabular_data)}[/TABULAR_DATA]"
...
async def stream_rag_response(query: str, context: str, llm_choice: Literal["openai", "gemini", "local", "groq", "biomistral", "openrouter"]):
    prompt = f"""
    ...
    if llm_choice == "groq":
        async def generate_rag_response():
            try:
                text = groq_llm.format_rag_response_groq(prompt)
                for char in text:
                    yield char
                yield "[DONE]"
            except Exception as e:
    ...
    if llm_choice == "biomistral":
        async def generate_rag_response():
            try:
                text = biomistral_llm.format_rag_response_biomistral(prompt)
                for char in text:
                    yield char
                yield "[DONE]"
            except Exception as e:
    ...
    if llm_choice == "openrouter":
        async def generate_rag_response():
            try:
                text = openrouter_llm.format_rag_response_openrouter(prompt)
                for char in text:
                    yield char
                yield "[DONE]"
            except Exception as e:
