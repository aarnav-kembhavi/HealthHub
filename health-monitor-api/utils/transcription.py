...
# Initialize Supabase client (uses placeholder URL if not set, so app can start)
supabase_client: Client = create_client(os.getenv("SUPABASE_URL"), os.getenv("SUPABASE_KEY"))

# Initialize Cohere only when API key is set
_cohere_api_key = os.getenv("COHERE_API_KEY")
co = cohere.ClientV2(api_key=_cohere_api_key) if _cohere_api_key else None

# Initialize LLM models only when API keys are set (allows app to start without them)
_openai_key = os.getenv("OPENAI_API_KEY")
_google_key = os.getenv("GOOGLE_API_KEY")
openai_model = ChatOpenAI(model_name="gpt-4o-mini", temperature=0.5) if _openai_key else None
gemini_model = ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=0.5) if _google_key else None

def generate_llm_response(prompt: str, context: str, model: str = "openai") -> str:
    if model == "openai" and openai_model is None:
        return "OpenAI API key not configured. Set OPENAI_API_KEY in .env to use this model."
    if model == "gemini" and gemini_model is None:
        return "Google API key not configured. Set GOOGLE_API_KEY in .env to use this model."
